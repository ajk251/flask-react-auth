...
def test_encode_token_None(test_app, test_database, add_user):

    user = add_user('justatest', 'test@test.com', 'test')
    token = user.encode_token(user.id, None)

    assert isinstance(token, str)

def test_decode_token_None(test_app, test_database, add_user):
