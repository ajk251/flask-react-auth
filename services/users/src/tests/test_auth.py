...
def test_user_registration(test_app, test_database):
    ...
    resp = client.post("/auth/register", 
                       data=json.dumps({"username": "justatest",
                                        "email": "test@test.com",
                                        "password": "123456"}),
...
def test_user_registration_invalid_json(test_app, test_database, payload):
    ...
    client = test_app.test_client()
    resp = client.post(f"/auth/register", data=json.dumps(payload), content_type="application/json",)
    data = json.loads(resp.data.decode())
...
def test_valid_refresh(test_app, test_database, add_user):
    ...
    resp_login = client.post("/auth/login",
                             data=json.dumps({"email": "test4@test.com",
                                              "password": "test"}),
                             content_type="application/json",
    )
    # valid refresh
...
def test_invalid_status(test_app, test_database):
    ...
    client = test_app.test_client()
    resp = client.get("/auth/status", headers={"Authorization": "Bearer invalid"},  content_type="application/json",)
    data = json.loads(resp.data.decode())
